...
def CCC(calData, refData, nrR, nrC, calData_nbp, refData_nbp, mask, mask_nbp):
    ...
    for i in range(nrR):
        for j in range(nrC):
            ...
            if number_of_pixels_in_patch > 0:
                # x - reference data nbp 10x10 masked
                # y - calculated data nbp 10x10 masked
                x_mean = numpy.mean(refData_nbp_10x10_masked)
                x_sd = numpy.std(refData_nbp_10x10_masked)
                csd_refData_patch = ((number_of_pixels_in_patch - 1) * x_sd ** 2) + (number_of_pixels_in_patch * x_mean ** 2)

                y_mean = numpy.mean(calData_nbp_10x10_masked)
                y_sd = numpy.std(calData_nbp_10x10_masked)
                csd_calData_patch = ((number_of_pixels_in_patch - 1) * y_sd ** 2) + (number_of_pixels_in_patch * y_mean ** 2)

                variance_refData_patch = (csd_refData_patch - (number_of_pixels_in_patch * x_mean ** 2)) / (number_of_pixels_in_patch - 1)
                variance_calData_patch = (csd_calData_patch - (number_of_pixels_in_patch * y_mean ** 2)) / (number_of_pixels_in_patch - 1)

                correlation = numpy.corrcoef(refData_nbp_10x10_masked, calData_nbp_10x10_masked, rowvar=1)[1][0]
                covariance = correlation * numpy.sqrt(variance_calData_patch) * numpy.sqrt(variance_refData_patch)
                ccc = (2 * covariance) / (variance_calData_patch + variance_refData_patch + (x_mean - y_mean) ** 2)
